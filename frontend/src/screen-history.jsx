  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        <span className={`dot ${run.status === 'resolved' || run.status === 'clean' ? 'ok' : run.status === 'failed' ? '' : ''}`}
              style={{ background: m.color, flexShrink: 0 }} />
        <div style={{ minWidth: 0 }}>
          <div className="mono" style={{ fontSize: 12, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {run.repo}
          </div>
          <div className="mono" style={{ fontSize: 10.5, color: 'var(--fg-faint)', marginTop: 1 }}>{run.id}</div>
        </div>
      </div>
      ...
      <div className="mono" style={{ fontSize: 12, textAlign: 'right', color: run.findings > 0 ? 'var(--red-soft)' : 'var(--fg-dim)' }}>
        {run.findings > 0 ? run.findings : '—'}
      </div>
      <div className="mono" style={{ fontSize: 12, textAlign: 'right', color: run.fixed > 0 ? 'var(--ok)' : 'var(--fg-dim)' }}>
        {run.fixed > 0 ? run.fixed : '—'}
      </div>
      <div className="mono" style={{ fontSize: 12, textAlign: 'right', color: 'var(--fg-dim)' }}>{run.dur}</div>
      <div className="mono" style={{ fontSize: 12, textAlign: 'right', color: 'var(--fg-dim)' }}>{run.cost}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {['all', 'resolved', 'partial', 'clean', 'failed'].map((f) => {
          const m = STATUS_META[f];
          return (
            <button key={f} onClick={() => setFilter(f)} style={{
              height: 26, padding: '0 12px', borderRadius: 999, fontSize: 11.5, fontWeight: 500,
      ...
      <div style={{
        ...
        {['Repository', 'When', 'Status', 'Found', 'Fixed', 'Duration', 'Cost'].map((h) => (
          <div key={h} style={{
            fontSize: 10.5, fontWeight: 600, letterSpacing: '.08em',
            color: 'var(--fg-faint)', textTransform: 'uppercase', textAlign: h !== 'Repository' ? 'right' : 'left',
          }}>{h}</div>
        ))}
      </div>
      ...
      <div style={{
        ...
        gap: 0, borderTop: '1px solid var(--line)', flexShrink: 0,
      }}>
        {[
          ...
          ['Spent', `$${HISTORY.reduce((a, r) => a + parseFloat(r.cost.replace('$', '')), 0).toFixed(2)}`],
        ].map(([k, v], i) => (
          <div key={k} style={{
            padding: '10px 16px', borderRight: i < 4 ? '1px solid var(--line)' : 0,
          }}>
            <div style={{ fontSize: 10.5, color: 'var(--fg-dim)', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.06em' }}>{k}</div>
