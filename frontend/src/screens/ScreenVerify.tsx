...
type PhaseStatus = 'waiting' | 'running' | 'done' | 'failed';
...
export function ScreenVerify({ runState, scenario, onExportSARIF }: Props) {
  const t = runState.t;
  ...
  const phases: Phase[] = [
    {
      ...
      result:
        isCrash && t > 5500
          ? 'FAILED: malformed JSON · scheduler retrying'
          : t > 7900
          ? 'CONFIRMED: 2 critical (JNS-001, JNS-002) + 1 false-positive (JNS-003)'
          : null,
      detail: [
    ...
    >
        ...
        >
          ...
          <div>
            <div style={{ fontWeight: 600, color: 'var(--ok)', fontSize: 15 }}>
              Audit complete — all critical findings resolved
            </div>
          ...
          <button
            ...
            style={{ borderColor: 'rgba(126,231,135,.3)', color: 'var(--ok)' }}
          >
      ...
      {failed && (
        ...
        >
          ...
          <div>
            <div style={{ fontWeight: 600, color: 'var(--red)', fontSize: 15 }}>
              Run failed — Janus_Red emitted invalid JSON
            </div>
            <div className="mono" style={{ fontSize: 12, color: 'var(--fg-dim)', marginTop: 3 }}>
      ...
      {isLimit && t > 13000 && (
        ...
        >
          ...
          <div>
            <div style={{ fontWeight: 600, color: 'var(--warn)', fontSize: 15 }}>
              Patch bounced — entering cycle 2 of 3
            </div>
            <div className="mono" style={{ fontSize: 12, color: 'var(--fg-dim)', marginTop: 3 }}>
            ...
            <div key={phase.key}>
              ...
              <div
                style={{
                  ...
                  opacity: phase.status === 'waiting' ? 0.45 : 1,
                  transition: 'all .4s',
                }}
              >
                <div
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    <span
                      style={{
                        fontWeight: 600,
                        fontSize: 13,
                        color: phase.status !== 'waiting' ? 'var(--fg)' : 'var(--fg-dim)',
                      }}
                    >
                      {phase.label}
                    </span>
                    ...
                    <div
                      style={{
                        ...
                        background:
                          phase.status === 'failed'
                            ? 'var(--red-bg)'
                            : phase.agent === 'red' && phase.key === 'red2'
                            ? 'var(--ok-bg)'
                            : phase.bg,
                        border: `1px solid ${
                          phase.status === 'failed'
