...
import { KV, Section } from '../components/atoms';
import { UtilBar } from '../components/Chrome';
import {
  startScan,
  useFileTree,
  useRepo,
...
const SCOPE_OPTIONS = ['SQLi', 'SSRF', 'XSS', 'RCE', 'Path traversal', 'Auth bypass', 'IDOR', 'Deserialization'];

function readTargetFromQuery(): string | null {
  if (typeof window === 'undefined') return null;
  const t = new URLSearchParams(window.location.search).get('target');
  return t ? t.trim() : null;
}

export function ScreenScan({ runState, scenario, setScenario }: Props) {
  const repo = useRepo();
  const fileTree = useFileTree();
  const system = useSystem();
  const runError = useRunError();
  const runStatus = useRunStatus();
  const queryTarget = readTargetFromQuery();
  const [url, setUrl] = useState(
    queryTarget ?? (repo.url ? `https://${repo.url}` : 'github.com/openwing-labs/vuln-flask-app'),
  );
  const [scope, setScope] = useState<string[]>(['SQLi', 'SSRF', 'XSS', 'RCE', 'Path traversal']);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      // error surfaced via useRunError below
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const sectionLabel = mode === 'repo' ? 'Repository' : mode === 'dir' ? 'Local directory' : 'File path';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
        <div>
          ...
          <p style={{ margin: '6px 0 0', color: 'var(--fg-mid)', maxWidth: 560, fontSize: 13.5 }}>
            Point Openwing at any GitHub repository, local directory, or single file. The Red Team agent traces user
            input through the codebase and tries to break it; the Blue Team agent patches what it finds, then hands
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span
                  style={{
                    ...
                    height: 10,
                    borderRadius: 999,
                    border: `2px solid ${mode === m.id ? 'var(--accent)' : 'var(--fg-faint)'}`,
                    background:
                      mode === m.id
                        ? 'radial-gradient(circle, var(--accent) 30%, transparent 35%)'
                        : 'transparent',
                  }}
                />
                <span style={{ fontWeight: 500, fontSize: 13 }}>{m.label}</span>
              </div>
              <div className="mono" style={{ fontSize: 10.5, color: 'var(--fg-dim)', marginTop: 4 }}>
                {m.sub}
              </div>
            </button>
          ))}
        ...
        <Section label={sectionLabel}>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              <span>{isRunning ? 'Scanning…' : submitting ? 'Starting…' : 'Start audit'}</span>
              <span
                className="kbd"
                style={{
          ...
          >
            {(
              [
                ...
                ['files', String(repo.files ?? 0)],
                ['LOC', (repo.loc ?? 0).toLocaleString()],
                ['size', repo.size || '—'],
              ] as const
            ).map(([k, v]) => (
              <div
              ...
              >
                ...
                <div className="mono" style={{ fontSize: 16, color: 'var(--fg)', marginTop: 2 }}>
                  {v}
                </div>
              </div>
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <Section label="Loop & runtime policy">
                  ...
                  {[1, 4, 8, 16].map((n) => (
                    <button
                      ...
                      style={{
                        ...
                        color: n === concurrency ? 'var(--accent)' : 'var(--fg-mid)',
                        cursor: 'pointer',
                      }}
                    >
                      {n}×
            ...
            <KV
              k="Network egress"
              v={
                <span className="pill warn" style={{ height: 18 }}>
                  SANDBOXED
                </span>
              }
            />
        ...
        <Section
          ...
          right={
            <span className="mono" style={{ fontSize: 11, color: 'var(--fg-dim)' }}>
              switch this to preview different agent paths
            </span>
          }
        >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
        ...
        >
          <div style={{ overflow: 'auto', padding: '6px 0', flex: 1 }}>
            {fileTree.map((f) => {
              ...
              const dot =
                status === 'critical' ? 'var(--red)' : status === 'flagged' ? 'var(--warn)' : 'var(--fg-faint)';
              return (
                <div
                  key={f.path}
                  style={{
          ...
          >
            <span style={{ color: 'var(--fg-dim)' }}>
              <span style={{ color: 'var(--red)' }}>●</span> {fileTree.filter((f) => f.status === 'critical').length}{' '}
              critical
            </span>
            <span style={{ color: 'var(--fg-dim)' }}>
              <span style={{ color: 'var(--warn)' }}>●</span> {fileTree.filter((f) => f.status === 'flagged').length}{' '}
              flagged
            </span>
            <span style={{ color: 'var(--fg-dim)' }}>
              · clean: {fileTree.filter((f) => f.status === 'clean').length}
            </span>
          </div>
        </Section>
        ...
        <Section label="Cluster">
          ...
          <KV
            ...
            v={
              <span className="mono" style={{ fontSize: 11 }}>
                1× MI300X · 192 GB VRAM · 20 vCPU · 240 GB RAM
              </span>
            }
          />
          <KV
          ...
          <div style={{ marginTop: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
              ...
              <span className="mono" style={{ color: 'var(--fg-mid)' }}>
                ${system.budget.spent.toFixed(2)} / ${system.budget.total.toFixed(0)}
              </span>
            </div>
            <UtilBar value={(system.budget.spent / system.budget.total) * 100} />
          </div>
        </Section>
      </div>
    </div>
