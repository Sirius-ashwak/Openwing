...
import { CodeBlock, Section, SeverityPill } from '../components/atoms';
...
function FindingCard({
  ...
  const patched = f.patchedAt != null && runState.t > f.patchedAt;
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
        <div style={{ minWidth: 0 }}>
          <div className="mono" style={{ fontSize: 11, color, letterSpacing: '.06em', fontWeight: 600 }}>
            {f.id}
          </div>
...
export function ScreenReport({ runState, onExportSARIF }: Props) {
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span className="mono" style={{ fontSize: 11, color: 'var(--fg-faint)' }}>
            findings will appear here once confirmed
          </span>
        </div>
      </div>
    );
  ...
  const patched = f.patchedAt != null && runState.t > f.patchedAt;
  const verified = f.verifiedAt != null && runState.t > f.verifiedAt;
  const cvssRows: ReadonlyArray<readonly [string, string]> = [
    ['Attack Vector', 'Network'],
    ['Attack Complexity', 'Low'],
    ['Priv Required', 'None'],
    ['User Interaction', 'None'],
    ['Confidentiality', 'High'],
    ['Integrity', f.id === 'JNS-002' ? 'Low' : 'High'],
  ];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', height: '100%', overflow: 'hidden' }}>
      <div style={{ borderRight: '1px solid var(--line)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        >
          <div
            className="mono"
            style={{ fontSize: 10.5, letterSpacing: '.08em', color: 'var(--fg-faint)', textTransform: 'uppercase' }}
          >
          ...
          <button className="btn sm" onClick={onExportSARIF}>
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
              <path
                d="M8 2v9m0 0L4 7m4 4 4-4M2 13h12"
                stroke="currentColor"
                strokeWidth="1.4"
                strokeLinecap="round"
        ...
        <div
          style={{ padding: 10, overflow: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}
        >
      ...
      <div style={{ overflow: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {f.impact.map((imp, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <span
                  style={{
                    color: f.severity === 'critical' ? 'var(--red)' : 'var(--fg-dim)',
                    marginTop: 2,
                    fontWeight: 700,
                  }}
                >
          ...
          <Section
            ...
            right={
              <span
                ...
                style={{
                  fontSize: 13,
                  color: f.severity === 'critical' ? 'var(--red)' : 'var(--warn)',
                  fontWeight: 600,
                }}
              >
                {f.cvss}
        ...
        <Section
          ...
          right={
            <span
              className="mono"
              style={{ fontSize: 11, color: f.severity === 'critical' ? 'var(--red)' : 'var(--fg-dim)' }}
            >
              {f.file}:{f.line}
            </span>
          }
        >
          <CodeBlock
            src={f.vulnCode}
            language="python"
            highlightLines={f.id === 'JNS-001' ? [14] : f.id === 'JNS-002' ? [11, 12, 13] : [1]}
          />
        </Section>

        {f.poc && (
          <Section
            label="Proof of concept exploit"
            right={<span className="pill red">generated by Janus_Red</span>}
          >
        ...
        {f.triage === 'false-positive' && (
          <Section label="Triage decision">
            ...
            >
              Janus_Red inspected the deploy pipeline and confirmed the production Dockerfile sets{' '}
              <span className="mono" style={{ color: 'var(--accent)' }}>
                DEBUG=False
              </span>{' '}
              explicitly. The default-true fallback is unreachable in production. Marked as{' '}
              <strong>false-positive</strong> · no patch required.
            </div>
          </Section>
        )}
      </div>
